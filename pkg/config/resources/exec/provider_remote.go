...
func (p *RemoteProvider) Create() error {
	p.log.Info("Remote executing script", "ref", p.config.Meta.ID)
	...
	if p.config.Target == nil {
		...
		if err != nil {
			return xerrors.Errorf("unable to create container for exec_remote.%s: %w", p.config.Meta.Name, err)
		}
...
func (p *RemoteProvider) Refresh() error {
	p.log.Debug("Refresh Remote Exec", "ref", p.config.Meta.ID)
