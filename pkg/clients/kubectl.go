	...
	for {
		...
		for _, pod := range pl.Items {
			if pod.Status.Phase != "Running" {
				allRunning = false
				k.l.Debug("Pod not running", "pod", pod.Name, "namespace", pod.Namespace, "status", pod.Status.Phase, "reason", pod.Status.Message)
				break
			}
			...
			for _, s := range pod.Status.ContainerStatuses {
				if !s.Ready {
					allRunning = false
					k.l.Debug("Pod not ready", "pod", pod.Name, "namespace", pod.Namespace, "container", s.Name, "status", s.State.String())
				}
			}
		}
